...
class PasGenerator(object):
    ...
    def print_functions(self, datatype):
        ...
        funcs += "static UA_INLINE UA_%s *\nUA_%s_new(void) {\n    return (UA_%s*)UA_new(%s);\n}\n\n" % (
            idName, idName, idName, PasGenerator.print_datatype_ptr(datatype))
    ...
    def print_enum_typedef(enum):
        ...
        ident=makeCIdentifier("UA_"+enum.name)    

        if enum.isOptionSet == True:
            return printsection("type") + "  "+ ident  + " = " +  enum.strDataType + ";\n" + \
               "  P" + ident + " = ^" + ident + ";\n" + printsection("const") + "\n".join(
                map(lambda kv: "  " + makeCIdentifier("UA_" + enum.name.upper() + "_" + kv[0].upper()) +
                " = " + kv[1]+ ";", values))
        else:
            return printsection("type") + "  "+ ident + " = (\n"+",\n".join(
                map(lambda kv: "    "+makeCIdentifier("UA_" + enum.name.upper() + "_" + kv[0].upper()) +
                               " = " + kv[1], values)) + \
                   ",\n    __UA_{0}_FORCE32BIT = $7fffffff\n".format(makeCIdentifier(enum.name.upper())) + "  );" + \
                   "\n  P" + ident + " = ^" + ident + ";\n" \
                   "  {$IF sizeof(UA_"+makeCIdentifier(enum.name)+") <> sizeof(UA_Int32)}{$MESSAGE ERROR 'enum_must_be_32bit'}{$IFEND}"
    ...
    def print_struct_typedef(struct):
        ...
        if struct.is_union:
            ...
            for member in struct.members:
                ...
                count += 1
            returnstr += PasGenerator.print_enum_typedef(obj)
            returnstr += "\n\n"
        if len(struct.members) == 0:
            return printsection("type")+"  UA_%s = pointer;" % makeCIdentifier(struct.name)
        returnstr += printsection("type") + "  UA_%s = record\n" % makeCIdentifier(struct.name)
        if struct.is_union:
        ...
        for member in struct.members:
            if member.is_array:
                returnstr += "    %sSize: size_t;\n" % makeCIdentifier(member.name)
                returnstr += "    %s: PUA_%s;\n" % (
                    makeCIdentifier(member.name), makeCIdentifier(member.member_type.name))
        ...
        if struct.is_union:
            #returnstr += "    } fields;\n"
            returnstr += "FIXME"
        return returnstr + "  end;\n  PUA_%s = ^UA_%s;" % (makeCIdentifier(struct.name), makeCIdentifier(struct.name))

    @staticmethod
    def print_datatype_typedef(datatype):
        if isinstance(datatype, EnumerationType):
            return PasGenerator.print_enum_typedef(datatype)
        if isinstance(datatype, OpaqueType):
            return printsection("type") + "  UA_%s = " % datatype.name + "UA_" + datatype.base_type +  ";"
        if isinstance(datatype, StructType):
            return PasGenerator.print_struct_typedef(datatype)
        raise RuntimeError("Type does not have an associated typedef")

    def write_definitions(self):
        self.fh = open(self.outfile + "_generated.inc", 'w')
    ...
    def print_header(self):
        self.printh('''(* Generated from ''' + self.inname + ''' with script ''' +
                    sys.argv[0] + '''\n* on host ''' + platform.uname()[1] + ''' by user ''' +
 ...
 * binary encoding, ...). *)''')
        ...
        self.printh(printsection("const")+"  UA_" + self.parser.outname.upper() + "_COUNT = %s;" % (str(totalCount)))
        ...
        if totalCount > 0:
            ...
            for ns in self.filtered_types:
                for i, t_name in enumerate(self.filtered_types[ns]):
                    ...
                    self.printh(
                        printsection("const")+"  UA_" + makeCIdentifier(self.parser.outname.upper() + "_" + t.name.upper()) + " = " + str(i)+";")
...
#include "''' + self.parser.outname + '''_generated.h"''')
        ...
        for ns in self.filtered_types:
            ...
            for i, t_name in enumerate(self.filtered_types[ns]):
                ...
                self.printc("")
                self.printc("/* " + t.name + " */")
                self.printc(PasGenerator.print_members(t, self.namespaceMap))
